...
var debug = require('debug')('loopback:connector:readsplitmysql');
...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  dataSource.connector = new ReadSplitMysql(dataSource.settings);
...
exports.ReadSplitMysql = ReadSplitMysql;
...
/**
 * @constructor
 * Constructor for ReadSplitMysql connector
 * @param {Object} client The node-mysql client object
 */
function ReadSplitMysql(settings) {
  SqlConnector.call(this, 'readsplitmysql', settings);
}

require('util').inherits(ReadSplitMysql, SqlConnector);

ReadSplitMysql.prototype.connect = function(callback) {
  var self = this;
  ...
  if (this.writeClient) {
    if (callback) {
      process.nextTick(function() {
        callback(null, self.writeClient);
      });
    }
  } else {
    const writeRatio = options.readonly ? options.readonly.writeRatio : 1;
    const totalConnections = options.connectionLimit;
    const writeConnections = Math.ceil(totalConnections * writeRatio);
    const readConnections = totalConnections - writeConnections;

    // Establish writable connections
    debug(`Connecting to MySQL master with ${writeConnections} connections`);
    options.connectionLimit = writeConnections;
    this.writeClient = mysql.createPool(options);

    // Establish read-only connections
    if (readConnections > 0) {
      options.connectionLimit = readConnections;
      options.host = options.readonly.host;
      options.port = options.readonly.port;
      options.username = options.readonly.username;
      options.password = options.readonly.password;

      debug(`Connecting to MySQL slave at ${options.host} with ${readConnections} connections`);
      this.readClient = mysql.createPool(options);
    }

    this.client = this.writeClient;

    this.writeClient.getConnection(function(err, connection) {
      var conn = connection;
      if (!err) {
        if (self.debug) {
...
function generateOptions(settings) {
  ...
  if (s.url) {
    ...
    if (s.readonly) {
      options.readonly = {
        writeRatio: s.readonly.writeRatio || 0.3, // Fraction of connectionLimit to dedicate to write's
        host: s.readonly.host || options.host,
        port: s.readonly.port || options.port,
        user: s.readonly.user || options.user,
        password: s.readonly.password || options.password,
      };
    }
 ...
 */
ReadSplitMysql.prototype.executeSQL = function(sql, params, options, callback) {
  const self = this;
  const writeOperators = ['INSERT', 'CREATE', 'UPDATE', 'DELETE'];
  const isWriteOperation = writeOperators.find((writeOperator) => sql.includes(writeOperator)) !== undefined;
  ...
  debugEnabled && debug('SQL: %s, params: %j, isWriteOperation: %j', sql, params, isWriteOperation);
  ...
  if (transaction && transaction.connection && transaction.connector === this) {
    debugEnabled && debug('Execute SQL within a transaction');
...
ReadSplitMysql.prototype._modifyOrCreate = function(model, data, options, fields, cb) {
 ...
 */
ReadSplitMysql.prototype.replaceOrCreate = function(model, data, options, cb) {
 ...
 * @param {Function} [cb] The callback function
 */
ReadSplitMysql.prototype.save =
ReadSplitMysql.prototype.updateOrCreate = function(model, data, options, cb) {
...
ReadSplitMysql.prototype.getInsertedId = function(model, info) {
 ...
 */
ReadSplitMysql.prototype.toColumnValue = function(prop, val) {
  if (val == null) {
...
ReadSplitMysql.prototype._serializeObject = function(obj) {
  var val;
 ...
 */
ReadSplitMysql.prototype.fromColumnValue = function(prop, val) {
  if (val == null) {
    return val;
  }
  if (prop) {
 ...
 */
ReadSplitMysql.prototype.escapeName = function(name) {
  return this.writeClient.escapeId(name);
};
 ...
 */
ReadSplitMysql.prototype._buildLimit = function(model, limit, offset) {
  if (isNaN(limit)) {
    limit = 0;
  }
  if (isNaN(offset)) {
...
ReadSplitMysql.prototype.applyPagination = function(model, stmt, filter) {
 ...
 */
ReadSplitMysql.prototype.getPlaceholderForIdentifier = function(key) {
  return '??';
};
 ...
 * @returns {String} The place holder
 */
ReadSplitMysql.prototype.getPlaceholderForValue = function(key) {
  return '?';
};

ReadSplitMysql.prototype.getCountForAffectedRows = function(model, info) {
 ...
 */
ReadSplitMysql.prototype.disconnect = function(cb) {
  if (this.debug) {
    debug('disconnect');
  }
  if (this.writeClient) {
    this.writeClient.end((err) => {
      if (err) return cb(err);

      if (this.readClient) {
        this.readClient.end(cb);
      } else {
        cb && cb();
      }
    });
...
ReadSplitMysql.prototype.ping = function(cb) {
  this.execute('SELECT 1 AS result', cb);
};

ReadSplitMysql.prototype.buildExpression = function(columnName, operator, operatorValue,
    propertyDefinition) {
  if (operator === 'regexp') {
    if (operatorValue.ignoreCase)
      g.warn('{{ReadSplitMysql}} {{regex}} syntax does not respect the {{`i`}} flag');

    if (operatorValue.global)
      g.warn('{{ReadSplitMysql}} {{regex}} syntax does not respect the {{`g`}} flag');

    if (operatorValue.multiline)
      g.warn('{{ReadSplitMysql}} {{regex}} syntax does not respect the {{`m`}} flag');
...
require('./migration')(ReadSplitMysql, mysql);
require('./discovery')(ReadSplitMysql, mysql);
require('./transaction')(ReadSplitMysql, mysql);
